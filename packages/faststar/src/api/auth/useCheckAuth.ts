...
// From https://github.com/msgbyte/tushan/blob/master/packages/tushan/client/api/auth/useCheckAuth.ts
export const useCheckAuth = (): CheckAuth => {
    const { authProvider } = useFaststarContext()
    const logout = useLogout()
    const loginUrl = defaultAuthParams.loginUrl

    const checkAuth = useEvent((params: any = {}, logoutOnFailure = true, redirectTo = loginUrl) =>
        authProvider!
            .checkAuth(params)
            .then(() => {
