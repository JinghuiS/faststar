 ...
 *                    \-- data is Product
 */
export const useCreate = <RecordType extends BasicRecord = any, MutationError = unknown>(
    options: UseCreateOptions<RecordType, MutationError> = {}
): UseCreateResult<RecordType, boolean, MutationError> => {
    const dataProvider = useDataProvider()
    const queryClient = useQueryClient()

    const mutation = useMutation<RecordType, MutationError, UseCreateMutateParams<RecordType>>({
        ...options,
        mutationFn: ({ resource, data, meta }) =>
            dataProvider
        ...
        onSuccess: (
            ...
            queryClient.setQueryData([resource, "getOne", { id: String(data.id) }], data)
...
> = UseMutationOptions<RecordType, MutationError, UseCreateMutateParams<RecordType>> & {
...
> = [
    ...
    UseMutationResult<RecordType, MutationError, UseCreateMutateParams<RecordType>, unknown>
]
