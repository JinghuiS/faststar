 ...
 *                        \-- data is Product
 */
export const useDeleteMany = <RecordType extends BasicRecord = any, MutationError = unknown>(
    resource?: string,
    ...
    const updateCache = ({ resource, ids }: { resource: string; ids: Identifier[] }) => {
        ...
        queryClient.setQueriesData(
            ...
            (res: GetListResult | undefined) => {
                ...
                const recordWasFound = newCollection && newCollection.length < res.data.length
        ...
        queryClient.setQueriesData(
            ...
            (coll: RecordType[] | undefined) => (coll && coll.length > 0 ? updateColl(coll) : coll),
            { updatedAt }
        )
        ...
        queryClient.setQueriesData(
            ...
            (res: GetListResult | undefined) => {
                ...
                return recordWasFound
                    ? {
                          data: newCollection,
                          total: (res.total ?? 0) - (res.data.length - newCollection.length)
                      }
                    : res
            },
            { updatedAt }
...
export interface UseDeleteManyMutateParams<RecordType extends BasicRecord = any> {
    resource?: string
    ids?: RecordType["id"][]
    meta?: any
}
...
export type UseDeleteManyResult<RecordType extends BasicRecord = any, MutationError = unknown> = [
    (
        resource?: string,
        params?: Partial<DeleteManyParams<RecordType>>,
