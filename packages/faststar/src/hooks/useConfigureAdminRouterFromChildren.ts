...
import { type CustomRoutesProp, CustomRoute } from "../components/CustomRoute"
import { Resource, type ResourceProps } from "../components/resource"
import { type FaststarCategoryPathInfo, useMenuStore } from "../store/menu"
...
const useRoutesAndResourcesFromChildren = (children: FaststarChildren): RoutesAndResources => {
    // const { permissions, isReady } = usePermissions()
    const [routesAndResources, setRoutesAndResources] = useRoutesAndResourcesState(
        getRoutesAndResourceFromNodes(typeof children === "function" ? undefined : children)
    )
            ...
            if (typeof renderFunc === "function") {
                setRoutesAndResources(
                    getRoutesAndResourceFromNodes(
                        renderFunc({ accessControlList: [], permissionsRoutes: [] })
                    )
                )
            }
        }
...
const getRoutesAndResourceFromNodes = (
    ...
    Children.forEach(children, (element) => {
        ...
        if (element.type === Fragment) {
            const routesFromFragment = getRoutesAndResourceFromNodes(element.props.children, path)
            customRoutesWithLayout.push(...routesFromFragment.customRoutesWithLayout)
            customRoutesWithoutLayout.push(...routesFromFragment.customRoutesWithoutLayout)
        ...
        if (element.type === Category) {
            ...
            customRoutesWithoutLayout.push(...routesFromCategory.customRoutesWithoutLayout)
